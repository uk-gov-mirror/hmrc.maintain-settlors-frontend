...
class AddASettlorController @Inject()(
  ...
  def submitAnother(): Action[AnyContent] = standardActionSets.identifiedUserWithData {
    implicit request =>
      Ok
  }

  def submitComplete(): Action[AnyContent] = standardActionSets.identifiedUserWithData {
    implicit request =>
      Ok
  }
}
